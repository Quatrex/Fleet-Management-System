...
const Database = {
	...
	savePicture(object, method, id = '', actionCreater = {}) {
		data = new FormData();
		data.append('Image', $('#'.concat(method))[0].files[0]);
		data.append('Method', method);
		if (id != '') {
			data.append('Id', $('#'.concat(id)).value);
		}
		$.ajax({
			url: '../func/save2.php',
			type: 'POST',
			...
			success: function (returnArr) {
				...
				if (returnArr.includes('false')) {
					...
					$('.'.concat(method.substring(6))).prop('src', path);
				}
			},
		});
	},
